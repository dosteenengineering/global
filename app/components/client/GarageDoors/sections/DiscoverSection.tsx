...
function DoorContent({ door }: { door: DoorItem }) {
  ...
  return (
    <article className="max-h-screen overflow-y-clip pr-4">
      <h3 className="text-40 lg:text-55 text-white leading-[1.181818181818182] font-light tracking-[-0.02em] mb-40 lg:mb-50">
        {door.title}
      </h3>
      ...
      <p className="w-fit rounded-full border border-white/70 px-6 py-2 text-15 leading-[1.4] text-white mb-30">
        {door.idealFor}
      </p>

      <h4 className="text-24 text-white leading-[1.333333333333333] font-light tracking-[-0.02em] mb-25">
        {door.heading}
      </h4>
      ...
      <ul className="space-y-3">
        {door.points.map((point) => (
          ...
          >
            <span className="absolute left-0 top-[0.75em] h-[3px] w-[3px] rounded-full bg-white" />
            {point}
          </li>
        ))}
      </ul>
    ...
    <section className="relative py-100 lg:py-150">
      ...
      <div className="container relative z-10">
        <SectionTitle
          ...
          className="section-heading text-white max-w-[1290px] mb-50 uppercase"
        />
        ...
        <div className="border-t border-[#76A7FF] pt-40 lg:pt-50">
          <div className="grid grid-cols-1 xl:grid-cols-[400px_minmax(0,1fr)] 3xl:grid-cols-[489px_minmax(0,1fr)]">
                  ...
                  {data.doors.map((door) => {
                    ...
                    return (
                      <button
                        key={door.id}
                        type="button"
                        onClick={() => handleMenuClick(door.id)}
                        className={`group flex min-w-fit items-center justify-between gap-5 rounded-full px-4 py-3 text-left transition-colors duration-300 xl:mb-[15px] xl:w-full xl:rounded-none xl:px-0 ${
                          isActive ? "bg-white/10 xl:bg-transparent" : ""
                        }`}
                      >
                        <span
                          className={`text-15 lg:text-16 leading-[1.5] tracking-[-0.02em] transition-colors duration-300 ${
                        ...
                        <span
                          className={`hidden h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white transition-all duration-300 xl:flex ${
        ...
        {data.ctaData && (
          <div className="mt-100 bg-white/10 px-8 py-12 lg:px-50 lg:py-50 3xl:px-[55px] 3xl:py-[52px]">
            <h3 className="text-36 lg:text-55 text-white leading-[1.18] font-light tracking-[-0.02em] mb-25">
            ...
            <p className="text-19 text-white/90 leading-[1.526315789473684] font-light max-w-[1040px] mb-35">
