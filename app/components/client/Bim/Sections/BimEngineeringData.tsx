            ...
            {stats.map((stat,index) => (
              <motion.div variants={moveUp(index*0.2*index)} initial="hidden" whileInView={"show"} viewport={{ once: true }} className="relative">
                ...
                <div key={stat.id} className="flex flex-col w-full p-5 lg:p-0">
                  <span className="text-55 xl:text-[32px] 3xl:text-55 leading-[1.1818] text-primary mb-[5px] font-light">
                    {stat.value}
                  </span>
                  <span className={`text-paragraph text-description lg:!text-[16px] 3xl:!text-[19px] ${descMaxWidth} `}>
                    {stat.label}
                  </span>
                </div>
              </motion.div>
