...
//   eighthSection,
// }: {
//   seventhSection: AboutPageData["seventhSection"];
//   eighthSection: AboutPageData["eighthSection"];
// }) => {
...
//         <SectionTitle
//           text={seventhSection.title}
//           className="section-heading-90 text-secondary uppercase mb-7.5 md:mb-50 max-w-[22ch]"
//         />
...
//                   <img
//                     src="./assets/images/about/map-section/glass-1.png"
//                     alt=""
//                     className="w-full h-full"
//                   />
//                 </div>

//                 <h3 className="text-55 leading-[1.181818181818182] font-light relative z-10">
//                   {seventhSection.items[0].number.replace("+", "")}
//                   {seventhSection.items[0].number.includes("+") && (
//                     <span className="text-primary">+</span>
//                   )}
//                 </h3>
//                 <p className="text-description !text-paragraph relative z-1 tracking-[2%]">
//                   {seventhSection.items[0].value}
//                 </p>
//               </motion.div>
//               <motion.div
...
//                   <img
//                     src="./assets/images/about/map-section/glass-2-new.png"
//                     alt=""
//                     className="w-full h-full object-contain"
//                   />
//                 </div>
//                 <h3 className="text-55 leading-[1.181818181818182] font-light relative z-10 max-xl:px-3">
//                   {seventhSection.items[1].number.replace("+", "")}
//                   {seventhSection.items[1].number.includes("+") && (
//                     <span className="text-primary">+</span>
//                   )}
//                 </h3>
//                 <p className="text-description !text-paragraph relative z-1 tracking-[2%] max-xl:px-3 max-xl:max-w-[25ch]">
//                   {seventhSection.items[1].value}{" "}
//                 </p>
//               </motion.div>
//             </div>
//             <motion.div
...
//               className="relative aspect-[1109.98/537] w-full max-w-[1109.98px] overflow-visible"
//             >
//               <Image
...
//                     >
//                       <img
//                         src="./assets/images/about/map-section/map-point.svg"
//                         alt=""
//                         className="block h-full w-full object-contain hover:scale-105"
//                       />
//                     </button>
//                     <AnimatePresence>
...
//                           >
//                             <span className="opacity-0 select-none font-light leading-[1.5] text-description uppercase w-fit ml-auto block">
//                               {country.title}
//                             </span>
//                           </motion.div>
...
//                               <p className="!text-[12px] md:!text-19 text-white font-light leading-[1.5] text-description !text-base uppercase text-right w-full">
//                                 {country.title}
//                               </p>
//                             </div>
//                           </motion.div>
...
//                     key={item.title}
//                   >
//                     <p className="text-19 tracking-[-0.02em] font-light text-paragraph leading-[1.65] md:leading-[2.105263157894737]">
//                       {item.title}
//                     </p>
//                   </motion.div>
//                 ))}
...
//         <img
//           src="./assets/images/about/map-section/elipse.svg"
...
//         <img
//           src="./assets/images/about/map-section/elipse2.svg"
...
import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { fadeIn, moveRight, moveUp } from "@/app/components/motionVariants";
import { CategoryWithCountries } from "@/lib/services/getCountryMap";

interface ServingMapProps {
  seventhSection: { title: string };
  countries: CategoryWithCountries[];
}

// static, per design — not sourced from backend
const STATIC_PARTNER_TITLE = "Countries, Global Partners";
const STATIC_PARTNER_NUMBER = "10+";
...
const ServingMap = ({ seventhSection, countries }: ServingMapProps) => {
  const [activeCategoryIndex, setActiveCategoryIndex] = useState(0);
  const [activeCountry, setActiveCountry] = useState<string | null>(null);

  const activeCategory = countries[activeCategoryIndex];
  const visibleCountries = activeCategory?.countries ?? [];

  const togglePlaceBox = (countryId: string) => {
    setActiveCountry((current) => (current === countryId ? null : countryId));
  };
              ...
              {countries.map((category, index) => {
                ...
                return (
                  <motion.button
                    key={category._id}
                    type="button"
                    variants={moveRight(0.3 + index * 0.5)}
                    initial="hidden"
                    whileInView="show"
                    viewport={{ once: true }}
                    onClick={() => handleCategorySwitch(index)}
                    aria-pressed={isActive}
                    className={`relative overflow-hidden z-10 rounded-full px-[22px] md:px-60 py-[13px] 3xl:py-[14px] min-w-[139px] text-left cursor-pointer transition-opacity duration-300 ${
                    ...
                    <div className="absolute inset-0 w-full h-full z-1">
                      <AnimatePresence initial={false} mode="sync">
                        <motion.img
                          ...
                          src={
                            isActive
                              ? "./assets/images/about/map-section/glass-1.png"
                              : "./assets/images/about/map-section/glass-2-new.png"
                          }
                          alt=""
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                ...
                {visibleCountries.map((country) => (
                  <motion.div
                    ...
                    className={`absolute -translate-x-1/2 -translate-y-1/2 ${
                      activeCountry === country._id ? "z-[100]" : "z-50"
                    }`}
                    style={{
                      left: `${country.xValue}%`,
                      top: `${country.yValue}%`,
                  ...
                  >
                    <div className="relative h-[15px] w-[15px]">
                      <button
                        ...
                        className="relative z-20 flex h-full w-full cursor-pointer items-center justify-center
                         [animation:map-point-scale_2.4s_ease-in-out_infinite] hover:animate-none"
                        onClick={() => togglePlaceBox(country._id)}
                      >
                        <img
                          src="./assets/images/about/map-section/map-point.svg"
                          alt=""
                      ...
                      <AnimatePresence>
                        {activeCountry === country._id && (
                          <>
                            {/* Sibling 1: Static Glass Background */}
                            <motion.div
                              initial={{ opacity: 0, y: "-50%" }}
                              animate={{ opacity: 1, y: "-50%" }}
                              exit={{ opacity: 0, y: "-50%" }}
            ...
            <div className="border-t border-bdr-gray  xl:border-t-0 pt-7.5 md:pt-12 xl:pt-0 mt-7.5 md:mt-12 xl:mt-0">
              <div className="mb-50 flex gap-2 xl:block">
                <motion.h3
                  ...
                  className="text-55 leading-[1.181818181818182] font-medium text-primary"
                >
                  {activeCategory?.count}<span className="text-primary">+</span>
                </motion.h3>
                <SectionTitle
              ...
              <div className="grid grid-cols-2 max-lg:max-w-[50%]">
                {activeCategory?.countries.map((country, index) => (
                  <motion.div
                    variants={moveUp(0.2 + index * 0.1)}
                    initial="hidden"
                    whileInView="show"
