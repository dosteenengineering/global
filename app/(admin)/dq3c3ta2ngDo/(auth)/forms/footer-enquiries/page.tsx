// app/(admin)/admin/forms/footer-enquiries/page.tsx
"use client";
...
const FooterEnquiriesPage = () => {
  const [entries, setEntries] = useState<FooterCallbackEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<string[]>([]);

  const fetchEntries = async () => {
    try {
      const res = await fetch("/api/admin/footer-callback");
      if (res.ok) {
        const data = await res.json();
        setEntries(data.data);
      }
    } catch (error) {
      console.log("Error fetching footer enquiries", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/footer-callback/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const res = await fetch("/api/admin/footer-callback/bulk-delete", {
        method: "DELETE",
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Footer Enquiries</h1>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-100 text-left">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold">Solution Type</th>
            ...
            <tbody>
              {entries.map((entry, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">{entry.solutionType || "—"}</td>
...
export default FooterEnquiriesPage;
