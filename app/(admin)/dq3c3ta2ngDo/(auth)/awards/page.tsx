...
import AwardCard from "@/app/components/AdminAward/AwardCard";
...
export default function Awards() {
  ...
  const confirmAwardOrder = async () => {
    try {
      const formData = new FormData();
      formData.append("awards", JSON.stringify(awardList.map((a) => a._id)));

      const response = await fetch("/api/admin/award/reorder", {
        method: "POST",
        body: formData,
      });
      ...
      console.log("Error reordering awards", error);
          ...
          <div className="flex justify-between border-b-2 border-black/20 pb-2">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  reorderMode ? confirmAwardOrder() : setReorderMode(true)
                }
              >
                {reorderMode ? "Done" : "Reorder"}
              </Button>
              <Dialog>
                <DialogTrigger className="bg-black text-white px-2 py-1 rounded-md">
                  Add Award
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          <DialogClose
                            className="bg-black text-white px-2 py-1 rounded-md"
                            onClick={() => handleEditAward(item._id)}
                          >
                            Save
                          </DialogClose>
                        </div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <div className="flex gap-2">
                          ...
                          <DialogClose
                            className="bg-black text-white px-2 py-1 rounded-md"
                            onClick={() => handleDeleteAward(item._id)}
                          >
                            Yes
                          </DialogClose>
                        </div>
